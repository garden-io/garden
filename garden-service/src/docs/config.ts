...
const moduleTypes = [
  ...
  { name: "openfaas", schema: openfaasModuleSpecSchema },
...
function renderMarkdownTitle(description: NormalizedDescription) {
  ...
  return parentDescriptions.length > 0
    ? `${parentDescriptions.map(d => d.name).join(".")}.${description.name}`
    : description.name
}
...
function makeMarkdownDescription(description: NormalizedDescription) {
  ...
  if (description.formattedExample) {
    formattedExample = renderSchemaDescriptionYaml(
      ...
      { showComment: false, useExampleForValue: true },
...
export function renderSchemaDescriptionYaml(
  descriptions: NormalizedDescription[],
  { showComment: showComments = true, showRequired = true, useExampleForValue = false }: RenderOpts,
) {
  ...
  const output = descriptions.map(desc => {
    ...
    // Prepend new line if applicable (easier then appending)
    if (prevDesc) {
    ...
    if (example && useExampleForValue) {
      ...
      value = isPrimitive
        ...
        : value = defaultValue === undefined ? "" : indent(safeDump(defaultValue).trim().split("\n"), 1)
    }
...
export function renderConfigReference(configSchema: Joi.ObjectSchema) {
  const partialTemplatePath = resolve(__dirname, "templates", "config-partial.hbs")
...
function renderProviderReference(schema: Joi.ObjectSchema, name: string) {
  ...
  const configReference = renderConfigReference(schema)
  const template = handlebars.compile(readFileSync(providerTemplatePath).toString())
  return template({ name, configReference })
}
...
function renderBaseConfigReference() {
  ...
  const projectSchemaReference = renderConfigReference(baseProjectSchema)
  const moduleSchemaReference = renderConfigReference(baseModuleSchema)

  const template = handlebars.compile(readFileSync(baseTemplatePath).toString())
  return template({ projectSchemaReference, moduleSchemaReference })
}

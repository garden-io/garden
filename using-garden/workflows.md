...
You can run a workflow by running `garden run workflow <name>`, or have it [trigger automatically](workflows.md#triggers) via Garden Enterprise.
...
Note that skipped steps don't produce any outputs \(see the [step outputs](workflows.md#step-outputs) section below for more\). However, skipped steps are shown in the command log.

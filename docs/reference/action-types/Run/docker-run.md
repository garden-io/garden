---
title: "`docker-run` Run"
tocTitle: "`docker-run` Run"
---
...
Uses `docker run` to execute the specified command in a new, one-off container from the specified image.
...
The following keys are available via the `${actions.run.<name>}` template string key for `docker-run`
action.

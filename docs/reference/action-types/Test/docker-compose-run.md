---
title: "`docker-compose-run` Test"
tocTitle: "`docker-compose-run` Test"
---
...
Runs a test suite by using ` compose run` to execute the specified command in a new, one-off container from the specified Compose service.
...
The following keys are available via the `${actions.test.<name>}` template string key for `docker-compose-run`
action.

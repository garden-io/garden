...
Runs a test suite by using `docker run` to execute the specified command in a new, one-off container from the specified image.
...
`docker-run` actions also export values that are available in template strings. See the [Outputs](#outputs) section below for details.
...
spec:
  ...
  # The image name for the container to run. Should be a valid Docker image identifier.
  image:
```
...
The following keys are available via the `${actions.test.<name>}` template string key for `docker-run`
action.

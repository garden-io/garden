---
title: "`docker-compose-exec` Test"
tocTitle: "`docker-compose-exec` Test"
---
...
Runs a test suite by using `docker compose exec` to execute the specified command in an already running `docker-compose` service.
...
`docker-compose-exec` actions also export values that are available in template strings. See the [Outputs](#outputs) section below for details.
...
The following keys are available via the `${actions.test.<name>}` template string key for `docker-compose-exec`
action.

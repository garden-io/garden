...
# DEPRECATED - Please use the `providers` field instead, and omit the environments key in the
# configured provider to use it for all environments, and use the `variables` field to configure
...
environmentDefaults:
  ...
  providers:
    ...
    - name:
      ...
      # empty array effectively disables the provider. To use a provider in all environments, omit
      # this field.
      environments:
  ...
  # we simply ignore it. If you do override the default value and the file doesn't exist, an error
  # will be thrown.

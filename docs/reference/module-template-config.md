...
{% hint style="info" %}
Module templates feature has been deprecated in `0.13` in favor of [Config Templates](../features/config-templates.md). We recommend updating your existing `ModuleTemplate` resources to `ConfigTemplate` resources and use the new `RenderTemplate` kind to render the templates. For backwards compatibility, the older declarations will still work until version `0.14`, and are converted at runtime.
{% endhint %}

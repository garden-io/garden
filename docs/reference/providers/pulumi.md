...
providers:
  - # The name of the provider plugin to use.
    ...
    # overrides this provider-level option. Works only with Pulumi service managed state backends.
    orgName:

    # The endpoint used to store Pulumi state backend. This option can also be set on the module level,
    # in which case it overrides this provider-level option (works with Pulumi versions 3.64.0 and above).
    #

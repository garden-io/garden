...
Starting from version 0.13 Garden will no longer support the cert-manager extension (previously built-in Garden by default), this means that you will need to implement your own cert-manager solution if you're not using the one built-in your Cloud Provider.
...
⚠️ For your convenience, this project is also available as a [full-code example.](https://github.com/garden-io/garden/tree/main/examples/cert-manager-ext-dns)
...
Note: *If you are using Dockerhub*, --docker-server should be: `https://index.docker.io/v1/`

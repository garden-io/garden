...
The [local Kubernetes](../plugins/kubernetes/local-k8s/README.md) is usually the easiest way to get started. All you need is a local Kubernetes installation (e.g. Minikube or Docker for Desktop) and a project to deploy.
...
To use the [Kubernetes plugin](../plugins/kubernetes/remote-k8s/README.md) you'll need access to a Kubernetes cluster so it may require a bit of upfront work.
...
The [Terraform plugin](../plugins/terraform/README.md) is usually used in conjunction with the Kubernetes plugin to provision infrastructure and/or cloud managed services such as databases.
...
The [Pulumi plugin](../plugins/pulumi/README.md) is very similar to the Terraform plugin (see above) except for use with Pulumi.

...
See [this section](https://docs.garden.io/v/acorn-0.12/using-garden/configuration-overview#multiple-modules-in-the-same-file) of our docs for more.
...
You can use the `disabled` field to disable [modules](https://docs.garden.io/v/acorn-0.12/using-garden/modules#disabling-modules), [services](https://docs.garden.io/v/acorn-0.12/using-garden/services#disabling-services), [tests](https://docs.garden.io/v/acorn-0.12/using-garden/tests#disabling-tests), and [tasks](https://docs.garden.io/v/acorn-0.12/using-garden/tasks#disabling-tasks).
...
Read all about it in [this section](https://docs.garden.io/v/acorn-0.12/using-garden/configuration-overview#including-excluding-files-and-directories) of our docs.
...
You can use the [copy directive](https://docs.garden.io/v/acorn-0.12/reference/module-types/container#build-dependencies-copy) of the `build.dependencies[]` field for that. See e.g. [this example project](https://github.com/garden-io/garden/tree/0.12.70/examples/build-dependencies).
...
Use the [`targetImage` field](https://docs.garden.io/v/acorn-0.12/reference/module-types/container#build-targetimage).

### How do I use base images?

See [this example project](https://github.com/garden-io/garden/tree/0.12.70/examples/base-image).
...
If you need the Dockerfile outside of the module root because you want to share it with other modules, you should consider having a single base image instead and then let each module have its own Dockerfile that's built on the base image. See the [base image example project](https://github.com/garden-io/garden/tree/0.12.70/examples/base-image) for an example of this.

### How do I include files/dirs (e.g. shared libraries) from outside the module root with the build context?

See [this example project](https://github.com/garden-io/garden/tree/0.12.70/examples/build-dependencies).

### How do I add Docker specific flags to the build command?

Use the module-level [`extraFlags` field](https://docs.garden.io/v/acorn-0.12/module-types/container#extraflags).
...
See also the [base image example project](https://github.com/garden-io/garden/tree/0.12.70/examples/base-image) for an example of this.
...
You can, however, run multiple [Kaniko pods](https://docs.garden.io/v/acorn-0.12/kubernetes-plugins/advanced/in-cluster-building#kaniko) in parallel. In some scenarios, that may scale better.
...
See [this section](https://docs.garden.io/v/acorn-0.12/kubernetes-plugins/advanced/in-cluster-building#pulling-base-images-from-private-registries) of our docs.

### How do I use my own private registry in in-cluster build mode?

See [this section](https://docs.garden.io/v/acorn-0.12/kubernetes-plugins/advanced/in-cluster-building#configuring-a-deployment-registry) of our docs.
...
Note that due to a [known limitation](https://github.com/garden-io/garden/issues/1739), Garden can't copy artifacts for tests in interactive mode. You can disable it by setting `--interactive false`. [See here](https://docs.garden.io/v/acorn-0.12/reference/commands#garden-run-test) for the full synopsis.
...
See [this section](https://docs.garden.io/v/acorn-0.12/k8s-plugins/module-types/container#secrets) of our docs.
...
No, Kubernetes secrets can only be used at runtime, by referencing them in the `environment` field of `tasks`, `services` and `tests`. See [the secrets section](https://docs.garden.io/v/acorn-0.12/k8s-plugins/module-types/container#secrets) of our docs for more.
...
See [this section](https://docs.garden.io/v/acorn-0.12/k8s-plugins/module-types/container#mounting-volumes) of our docs.

### How do I access files that are generated at runtime (e.g. migration files that are checked into version control)?

You can generate the files via a task, store them as artifacts, and copy them from the local artifacts directory. [Here's an example](https://docs.garden.io/v/acorn-0.12/using-garden/tests#test-artifacts) of this.

You can also use the [`persistentvolumeclaim`](https://docs.garden.io/v/acorn-0.12/reference/module-types/persistentvolumeclaim) module type to store data and share it across modules. See [this section](https://docs.garden.io/v/acorn-0.12/k8s-plugins/module-types/container#mounting-volumes) of our docs for more.

## Kubernetes

### How do I annotate ingresses?

You can set annotations on ingresses under the [`services[].ingresses[]` field](https://docs.garden.io/v/acorn-0.12/reference/module-types/container#services-ingresses-annotations).
...
No, you have to use the [`kubernetes`](https://docs.garden.io/v/acorn-0.12/reference/module-types/kubernetes) module type for that.
...
The components installed when using the remote building functionality are discussed in the [In-cluster building docs](https://docs.garden.io/v/acorn-0.12/kubernetes-plugins/advanced/in-cluster-building).

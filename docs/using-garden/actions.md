...
It also might be better for maintainability to have multiple actions of the same `kind` in the same configuration file,
e.g. `build.actions.garden.yml` or `test.actions.garden.yml`.

Another example is when the dev configuration and the production configuration have different integration testing
suites, which may depend on different external applications being available.

To do this, add a document separator (`---`) between the action definitions. Here's a simple (if a bit contrived)
example:

The Build actions can be listed in a separate file:
...
The tests configurations can be places in a separate file too:

```yaml
kind: Test
description: My container tests - configuration A
type: container
name: test-suite-a
build: image-a
...
# assume that the Test action below is declared elsewhere
dependencies:
  - test.a-integration-testing-backend
spec:
...
kind: Test
description: My container tests - configuration B
type: container
name: test-suite-b
build: image-b
...
# assume that the Test action below is declared elsewhere
dependencies:
  - test.b-integration-testing-backend
spec:

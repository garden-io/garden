...
import type { CacheableResult } from "../plugins/kubernetes/results-cache-base.js"
...
export class GardenCloudApi {
  ...
  async createActionResult(request: CreateCachedActionRequest): Promise<CreateCachedActionResponse> {
    try {
      ...
      throw new CloudApiError({
        message: `Error storing data to Team Cache; code=${e.code}; cause: ${rootCause}`,
  ...
  async getActionResult({
    ...
    actionType,
    cacheKey,
  }: GetCachedActionRequest): Promise<GetCachedActionResponse<CacheableResult>> {
    try {
      return await this.get<GetCachedActionResponse<CacheableResult>>(
        `/action-cache?projectId=${projectId}&schemaVersion=${schemaVersion}&actionRef=${actionRef}&actionType=${actionType}&cacheKey=${cacheKey}`
      )
    } catch (e) {
      if (!(e instanceof RequestError)) {
        throw e
      ...
      const rootCause = extractErrorMessageBodyFromGotError(e) ?? e.message
      throw new CloudApiError({
        message: `Error getting data from Team Cache; code=${e.code}; cause: ${rootCause}`,
        responseStatusCode: e.response?.statusCode,
      })
    }
  }
...
type CachedAction<T> = { found: true; result: T } | { found: false; notFoundReason: string }

export interface GetCachedActionResponse<T> extends BaseResponse {
  data: CachedAction<T>
}

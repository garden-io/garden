...
export const serverRequestSchema = createSchema({
  ...
  keys: () => ({
    ...
    // TODO @eysi: Make required if GARDEN_ENABLE_REMOTE_WS
    authKey: joi
      .string()
      .allow(null)
      .optional()
      .description("An auth key that's generated by Core and should be included with every request."),
    sessionId: joi
      .string()
      .allow(null)
      .optional()
      .description("The session ID that's generated by Core and should be included with every request."),
    command: joi
      .string()

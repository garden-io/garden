...
function normalizeContainers(manifestContainers: any[], deployedContainers: any[] | undefined) {
  ...
  for (let i = 0; i < manifestContainers.length; i++) {
    const manifestContainer = manifestContainers[i]
    const deployedContainer = deployedContainers?.[i]

    // imagePullPolicy defaults based on image tag
    if (manifestContainer.imagePullPolicy === undefined) {
      manifestContainer.imagePullPolicy = manifestContainer.image?.endsWith(":latest") ? "Always" : "IfNotPresent"
    }

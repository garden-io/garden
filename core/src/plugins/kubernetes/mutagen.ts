...
export class MutagenDaemon {
  ...
  private syncStatusLines: { [sessionName: string]: Log }
   ...
   */
  static async start({ ctx, log }: { ctx: PluginContext; log: Log }) {
      ...
      .then((sessions) => {
        for (const session of sessions) {
          ...
          if (!this.syncStatusLines[sessionName]) {
            this.syncStatusLines[sessionName] = this.log.makeNewLogContext({})
          }
          ...
          if (statusMsg) {
            this.syncStatusLines[sessionName].info({
              symbol: "info",
              section,

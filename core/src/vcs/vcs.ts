...
export function hashStrings(strings: string[], limit: number = SHORT_VERSION_HASH_LENGTH) {
  const versionHash = createHash("sha256")
  versionHash.update(strings.join("."))
  return versionHash.digest("hex").slice(0, limit)
}

export function fullHashStrings(strings: string[]) {
  return hashStrings(strings, undefined)
}

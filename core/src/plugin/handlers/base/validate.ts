...
> {
  description = dedent`
    ...
    When passed to this handler, be aware that the \`spec\` and \`variables\` fields will *not* be fully resolved, so validation on that field may not be advisable at this stage. Specifically, references to other actions (e.g. runtime outputs) will be resolved later.

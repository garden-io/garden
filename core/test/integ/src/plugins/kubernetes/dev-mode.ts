...
describe("dev mode deployments and sync behavior", () => {
  ...
  it("should apply ignore rules from the sync spec and the project-level .gardenignore-dev", async () => {
    await init("local")
    const service: ContainerService = graph.getService("dev-mode")

    // We want to ignore the following directories (all at module root)
    // prefix-a
    // somedir <--- in the project-level .gardenignore-dev
    // nested/prefix-b <--- in the module-level .gardenignore-dev
    service.spec.devMode!.sync[0].mode = "one-way-replica"
    service.spec.devMode!.sync[0].exclude = ["prefix-*"]
    const module = service.module
    const log = garden.log
    const deployTask = new DeployTask({
      garden,

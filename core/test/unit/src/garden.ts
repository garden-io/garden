      ...
      await expectError(
        ...
        (err) => {
          ...
          expect(stripAnsi(err.detail.messages[0])).to.equal(
            "- test: Invalid template string (${bla.ble}): Could not find key bla. Available keys: command, environment, datetime, git, local, project, providers, secrets, var and variables."
          )
        }
      )
    })

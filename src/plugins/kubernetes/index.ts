...
export class KubernetesProvider implements Plugin<ContainerModule> {
  ...
  async configureEnvironment(params: ConfigureEnvironmentParams) {
    // TODO: use Helm 3 when it's released instead of this custom/manual stuff
    const { ctx, env, logEntry } = params
  ...
  async deployService(params: DeployServiceParams<ContainerModule>) {
    return deployService(params)
  }

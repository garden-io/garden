...
for (const terraformVersion of ["0.13.3", "1.3.7"]) {
  describe(`Terraform provider with terraform version ${terraformVersion}`, () => {
    ...
    context("autoApply=false", () => {
      beforeEach(async () => {
        ...
        garden = await makeTestGarden(testRoot, {
          ...
          forceRefresh: true,
          variables: { "tf-version": terraformVersion },
        })
    ...
    context("autoApply=true", () => {
      ...
      it("sets the workspace before applying the stack", async () => {
        const _garden = await makeTestGarden(testRoot, {
          environmentName: "local",
          forceRefresh: true,
          variables: { "workspace": "foo", "tf-version": terraformVersion },
          plugins: [gardenPlugin()],
        })
  ...
  describe(`Terraform module type with terraform version ${terraformVersion}`, () => {

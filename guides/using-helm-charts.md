...
In this guide we'll be using the [vote-helm](https://github.com/garden-io/garden/tree/0.12.15/examples/vote-helm) project. If you prefer to just check out a complete example, the project itself is also a good resource.

You may also want to check out the full [helm module reference](../reference/module-types/helm.md).

_Note: If you only need a way to deploy some Kubernetes manifests and don't need all the features of Helm, you can_ _use the simpler `kubernetes` module type instead. Check out the_ [_kubernetes-module_](https://github.com/garden-io/garden/tree/0.12.15/examples/kubernetes-module) _example for more info._
...
In our base chart, we make certain values like `name`, `image.repository` and `image.tag` required \(using the [required](https://helm.sh/docs/howto/charts_tips_and_tricks/) helper function\) in order to enforce correct usage. We recommend enforcing constraints like that, so that mistakes can be caught quickly.
...
Check out the full [helm module reference](../reference/module-types/helm.md) for more details, and the [vote-helm](https://github.com/garden-io/garden/tree/0.12.15/examples/vote-helm) example project for a full project that showcases Garden's Helm support.

Also check out the [kubernetes-module](https://github.com/garden-io/garden/tree/0.12.15/examples/kubernetes-module) example for a simpler alternative, if you don't need all the features of Helm.

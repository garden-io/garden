...
* When using [in-cluster building](in-cluster-building.md) your CI also uses the same build and test result cache as you and your team, which makes for a much faster pipeline.

To use Garden in your CI pipeline you need the following:

1. A Garden project, [configured to deploy to a remote cluster](using-garden-in-ci.md#configure-remote-environments).
2. A [Kubectl context](using-garden-in-ci.md#configure-the-kubectl-context) on the CI agent that's configured for the remote cluster.
...
The project is based on our basic [demo-project](https://github.com/garden-io/garden/tree/0.12.15/examples/demo-project) example, but configured for multiple environments. Additionally it contains a CircleCI config file. You'll find the entire source code [here](https://github.com/garden-io/ci-demo-project).
...
Configuring Garden to work against a remote Kubernetes cluster is explained step by step in our [Remote Kubernetes guide](remote-kubernetes.md). For this example, we also use [in-cluster building](in-cluster-building.md).

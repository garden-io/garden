...
> **Remote** _**source**_: A repository that contains one or more Garden modules _and_ their corresponding `garden.yml` config files.
>
> **Remote** _**module**_: The source code for a single Garden module. In this case, the `garden.yml` config file is stored in the main project repository while the module code itself is in the remote repository.

The code examples below are from our [remote sources example](https://github.com/garden-io/garden/tree/0.12.15/examples/remote-sources).
...
Local paths work just the same remote URLs and you'll still need to [link the repository](using-remote-sources.md#linking-remote-sourcesmodules-to-local-code) if you want to edit it locally.

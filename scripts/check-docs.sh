...
git diff --quiet HEAD -- docs/ || (echo 'generated docs are not up-to-date! run \"npm run build && npm run generate-docs\" and commit the changes. If that doesn't help try npm run clean-build.\n' && exit 1)
